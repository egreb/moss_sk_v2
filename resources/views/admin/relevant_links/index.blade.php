...
@section('content')
    ...
    <form class="flex flex-col lg:flex-row w-full" method="POST" action="{{ route('admin.relevant_links.store') }}">
        @csrf
        <input type="text" name="description" value="{{ old('description') }}" placeholder="Beskrivelse" required/>
        <input type="text" name="url" value="{{ old('url') }}" placeholder="Link" required/>
        <input class="w-1/2" type="date" name="expires" value="{{ old('expires') }}"
