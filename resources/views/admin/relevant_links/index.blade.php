...
<form class="flex flex-col lg:flex-row w-full" method="POST" action="{{ route('admin.relevant_links.store') }}">
    @csrf
    <input class="lg:w-5/12" type="text" name="description" value="{{ old('description') }}" placeholder="Beskrivelse" required />
    <input class="lg:w-5/12" type="text" name="url" value="{{ old('url') }}" placeholder="Link" required />
    <input class="w-1/2 lg:w-2/12" type="datetime-local" name="expires" value="{{ old('expires') }}" placeholder="Utløper (Ikke påkrevd)" />
