...
<form class="w-full" method="post" action="{{ route('admin.post.store') }}" enctype="multipart/form-data">
    @csrf

    <post-image post-id="{{ isset($post) ? $post->id : null }}" image-id="{{ isset($post) && !is_null($post->image) ? $post->image->id : null }}" image-name="{{ isset($post) && !is_null($post->image) ? $post->image->name : null }}" image-url="{{ isset($post) && !is_null($post->image) ? $post->image->url() : null }}" image-srcset="{{ isset($post) && !is_null($post->image) ? $post->image->srcset() : null }}"></post-image>

    <div class="mb-4">
        <label class="block text-gray-700 text-sm font-bold mb-2" for="title">
            Tittel
        </label>
        <input class="shadow appearance-none border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline" id="title" type="text" placeholder="Tittel" name="title" value="{{ old('title') }}">
    ...
    <div class="flex justify-start items-center pb-10">
        <button class="bg-blue-500 hover:bg-blue-700 text-white font-bold py-2 px-4 rounded focus:outline-none focus:shadow-outline mr-6">
            Lagre
        </button>
